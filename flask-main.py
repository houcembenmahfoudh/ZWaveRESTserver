...
backend = Backend_with_switches_dimmers_and_sensors()
...
############## SWITCHES #####################################################################
#############################################################################################
### THESE METHODS WERE SPECIALLY MADE FOR SWITCH NODES ######################################
#############################################################################################

"""
@api {get} /switches/get_switches_list get_switches_list
@apiName get_switches_list
@apiGroup Actuators

@apiSuccess {String[]} JSON List of all switch nodes in the network i a JSON format
...
@apiDescription Lists IDs and product names of all switch nodes in the network. The controller is excluded.

"""

@app.route('/switches/get_switches_list', strict_slashes=False)
def switches():
    return backend.get_switches()


"""
@api {post} /switches/send_command command_switches
@apiName command_switches
@apiGroup Actuators

@apiParam {Number} node_id Switch's unique ID
@apiParam {String} value command value (on/off/check) 
...
@apiSuccess {String} command switch's new state

@apiDescription Sends command to switch node

"""

@app.route('/switches/send_command', methods=['GET', 'POST'], strict_slashes=False)
def switch():
    if request.method=='POST':
        content = request.get_json()
        if all(item in content.keys() for item in ['node_id','value']):
            node = int(content['node_id'])
            on_off_check = content['value']
            if on_off_check == 'on':
                ...
                return "switch %s (part 1) switched on" % node
                ...
                if val:
                    return "switch %s (part 1) is currently on" % node
                else:
                    return "switch %s (part 1) is currently off" % node
            elif on_off_check == 'on2':
                backend.switch_on2(node)
                return "switch %s (part 2) switched on" % node
            elif on_off_check == 'off2':
                backend.switch_off2(node)
                return "switch %s (part 2) switched off" % node
            elif on_off_check == 'check2':
                val = backend.get_switch_status2(node)
                if val:
                    return "switch %s (part 2) is currently on" % node
                else:
                    return "switch %s (part 2) is currently off" % node
            else:
...
if __name__ == '__main__':
    try:
        ...
        app.run(host='::', port=3671, debug=False, use_reloader=False, threaded=True)
